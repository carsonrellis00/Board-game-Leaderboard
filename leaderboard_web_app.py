...
def save_leaderboard_file(leaderboard, save_file):
    data = {name: (r.mu, r.sigma) for name, r in leaderboard.items()}
    with open(save_file, "w") as f:
        json.dump(data, f)

def save_history_file(history, history_file):
    with open(history_file, "w") as f:
        json.dump(history, f)

def record_game_web(history_file, leaderboard_file, teams, ranks):
    ...
    save_history_file(history, history_file)
    leaderboard = recalc_ratings(history)
    save_leaderboard_file(leaderboard, leaderboard_file)
    return leaderboard, history

# ---- Streamlit UI ----
st.title("Board Game Leaderboards (Web Input)")
...
if game_name:
    ...
    st.write("Enter teams in finishing order. Use ',' for players in the same team, '=' for tied teams, and ';' for different ranks.")
    example = "Alice,Bob=Charlie,David;Eve,Frank"
    st.write(f"Example input: `{example}`")
    game_input = st.text_input("Teams & ranks:", "")

    if st.button("Record Game"):
        if game_input.strip():
            rank_groups = [grp.strip() for grp in game_input.split(";") if grp.strip()]
            teams = []
            ranks = []
            for rank, group in enumerate(rank_groups):
                ...
                for team in tied_teams:
                    ...
                    if players:
                        ...
                        ranks.append(rank)
            if len(teams) >= 2:
                leaderboard, history = record_game_web(history_file, save_file, teams, ranks)
                st.success("Game recorded!")
            else:
                st.error("You must enter at least 2 teams.")
        else:
            st.error("Please enter teams and ranks.")
    ...
    with col1:
        if st.button("Wipe Leaderboard"):
            confirm = st.text_input("Type YES to confirm wiping leaderboard:", "")
            if confirm.strip().upper() == "YES":
                ...
                leaderboard = {}
                history = []
                st.success(f"Leaderboard for {game_name.title()} wiped!")
            elif confirm:
                st.warning("Type YES exactly to wipe.")

    with col2:
        if st.button("Undo Last Game"):
            if history:
                history.pop()
                save_history_file(history, history_file)
                leaderboard = recalc_ratings(history)
                save_leaderboard_file(leaderboard, save_file)
                st.success("Last game undone!")
            else:
                st.warning("No games to undo.")
